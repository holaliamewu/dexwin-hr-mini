...
export default function EditDepartmentModal({ open, onClose }: TModalProps) {
    ...
    return(
        <section className="flex flex-col items-center justify-center absolute w-screen h-full top-0 left-0 bg-black/70 z-100" >
            <div className="relative min-w-[640px] h-fit bg-white p-6 rounded-xl" >
                <h1 className="text-[20px] font-bold" >Edit Department</h1>
                <p className="text-[14px] text-[#71717a]  " >Make changes to your department here. Click save when you're done.</p>
                <form className="flex flex-col gap-6 mt-6" >
                    <span className="flex flex-col gap-2" >
                        <label className="text-[14px] font-semibold" >Name</label>
                        <Input placeholder="Enter department name" className=""  />
                    </span>
                    <span className="flex flex-col gap-2" >
                    ...
                    {!loading && (<span className="flex items-center justify-between" >
                        ...
                        <Button variant="default" className="min-w-[175px] py-[7.5px] px-6 py-2.5 text-sm bg-[#02AA69] " >Save Changes</Button>
                    </span>)}
                    { loading && (
