      ...
      <div className="flex flex-col items-center justify-center w-full max-w-[360px] " >
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col w-full gap-5" >
          <span className="flex flex-col w-full gap-1.5 " >
            ...
            <Input className={clsx("w-full h-[44px] text-sm", errors.email && "border border-red-400 focus:outline-solid outline-2")} placeholder="Enter your email" {...register("email")} />
            { errors.email && <p className="text-sm text-red-500 mt-1" >Kindly provide a valid email “you@gmail.com”</p> }
          </span>
          <span className="flex flex-col w-full gap-1.5 " >
            <label className="text-sm" >Password</label>
            <Input className="w-full h-[44px] text-sm" type="password" placeholder="Enter your password" {...register("password",)} />
          ...
          { !loading && (
            ...
            <button type="submit" className="w-full h-[44px] text-white text-md font-semibold bg-[#02AA69] px-3 rounded-lg shadow " >Sign in</button>
            <Button variant="outline" onClick={ () => { setLoading(true)}} className="w-full h-[44px] text-md px-3 rounded-lg shadow" >Sign in with Google</Button>
          </span>
        )}
          { loading && (
